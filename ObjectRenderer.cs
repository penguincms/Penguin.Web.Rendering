...
using Penguin.Web.Rendering.Abstractions;
using System;
...
namespace Penguin.Web.Rendering
{
    ...
    public class ObjectRenderer
    {
        ...
        protected static object BuildPageModel(IEnumerable<TemplateParameter> toGenerate)
        {
            ...
            List<TemplateParameter> modelParameters = toGenerate.ToList();

            int i = 0;
            while (modelParameters.Count < 2)
            {
                string name = "PlaceHolder" + $"{++i}";

                modelParameters.Add(new TemplateParameter(typeof(object), name, new object()));
            }
        ...
        {
            List<TemplateParameter> modelValues = toGenerate.ToList();
            ...
            lock (TemplateLock)
            {
                if (!File.Exists(AbsolutePath))
                {
                    ...
                    if (!contents.Contains(AUTOGENERATED_TAG))
                    {
                        ...
                        {
                            TUPLE_NOTE,
                            $"@model ({string.Join(", ", toGenerate.Select(p => $"{GetTypeDisplayName(p.Type)} {p.Name}"))})",
                            AUTOGENERATED_TAG,
                            $"@*{e.Guid}*@"
                        };
